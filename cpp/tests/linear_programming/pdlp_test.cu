...
//     solver_settings); auto pdlp_timer = timer_t(solver_settings.time_limit);
...
//   {
//     cuopt::linear_programming::detail::pdlp_solver_t<int, double> solver(problem,
//     solver_settings); auto pdlp_timer = timer_t(solver_settings.time_limit); std::vector<double>
//     initial_primal(op_problem.get_n_variables(), 1); auto d_initial_primal =
//     device_copy(initial_primal, handle_.get_stream());
...
//     cuopt::linear_programming::detail::pdlp_solver_t<int, double> solver(problem,
//     solver_settings); auto pdlp_timer = timer_t(solver_settings.time_limit); std::vector<double>
//     initial_dual(op_problem.get_n_constraints(), 1); auto d_initial_dual =
//     device_copy(initial_dual, handle_.get_stream());
...
//     solver_settings); auto pdlp_timer = timer_t(solver_settings.time_limit);
//     cuopt::linear_programming::pdlp_hyper_params::update_primal_weight_on_initial_solution =
//     true; solver.run_solver(pdlp_timer);
//     RAFT_CUDA_TRY(cudaStreamSynchronize(handle_.get_stream()));
//     EXPECT_NEAR(initial_step_size_afiro, solver.get_step_size_h(), factor_tolerance);
//     EXPECT_NEAR(initial_primal_weight_afiro, solver.get_primal_weight_h(), factor_tolerance);
//     cuopt::linear_programming::pdlp_hyper_params::update_primal_weight_on_initial_solution =
...
//     cuopt::linear_programming::detail::pdlp_solver_t<int, double> solver(problem,
//     solver_settings); auto pdlp_timer = timer_t(solver_settings.time_limit); std::vector<double>
//     initial_primal(op_problem.get_n_variables(), 0); auto d_initial_primal =
//     device_copy(initial_primal, handle_.get_stream());
//     solver.set_initial_primal_solution(d_initial_primal);
...
//     true; cuopt::linear_programming::detail::pdlp_solver_t<int, double> solver(problem,
//     solver_settings); auto pdlp_timer = timer_t(solver_settings.time_limit); std::vector<double>
//     initial_dual(op_problem.get_n_constraints(), 0); auto d_initial_dual =
//     device_copy(initial_dual, handle_.get_stream());
//     solver.set_initial_dual_solution(d_initial_dual);
//     solver.run_solver(pdlp_timer);
//     EXPECT_NEAR(initial_step_size_afiro, solver.get_step_size_h(), factor_tolerance);
...
//   {
//     cuopt::linear_programming::detail::pdlp_solver_t<int, double> solver(problem,
//     solver_settings); auto pdlp_timer                             =
//     timer_t(solver_settings.time_limit); constexpr double test_initial_step_size     = 1.0;
...
//     solver_settings); pdlp_timer = timer_t(solver_settings.time_limit);
