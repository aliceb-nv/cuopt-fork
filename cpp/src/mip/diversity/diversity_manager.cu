...
solution_t<i_t, f_t> diversity_manager_t<i_t, f_t>::run_solver()
{
  ...
  if (population.current_size() > 0) {
    ...
    printf("POPULATION FEASIBLE!!!! initial solution objective: %g\n",
    ...
    auto end = thrust::copy_if(problem_ptr->handle_ptr->get_thrust_policy(),
                               ...
                               vars_to_fix.begin(),
                               [lpopt     = lp_optimal_solution.data(),
    ...
    if (n_to_fix == 0) exit(0);

    CUOPT_LOG_DEBUG("Running RINS");
    CUOPT_LOG_DEBUG("Fixed %d variables", vars_to_fix.size());
    CUOPT_LOG_DEBUG("Fractional ratio %g%%", fractional_ratio * 100);
    ...
    f_t objective_cut = best_sol.get_objective() -
                        std::max(std::abs(0.001 * best_sol.get_objective()), OBJECTIVE_EPSILON);
    ...
    // branch_and_bound_settings.time_limit           = 20;
    ...
    CUOPT_LOG_DEBUG("RINS Solution: feasible: %d, objective: %g",
                    ...
                    best_sol.get_user_objective());
    // initial_sol_vector.push_back(std::move(best_sol));
  }
  exit(0);
