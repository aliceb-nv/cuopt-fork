...
{
  // printf("-------- new best incumbent callback w/ solution %d\n", (int)solution.size());
  ...
  if (node_count - node_count_at_last_rins > settings.node_freq) {
    ...
    if (!rins_thread->cpu_thread_should_start &&
        (dm.population.current_size() > 0 || dm.population.external_solution_queue.size() > 0 ||
         dm.population.external_solution_queue_cpufj.size() > 0)) {
      lp_optimal_solution = solution;
      rins_thread->start_cpu_solver();
    } else {
      // printf("-------- rins thread not done\n");
    }
  }
}
...
void rins_t<i_t, f_t>::run_rins()
{
  ...
  // RINS
  if (dm.population.solutions.size() == 0) { return; }
  if (dm.population.current_size() > 0 || dm.population.external_solution_queue.size() > 0 ||
      dm.population.external_solution_queue_cpufj.size() > 0) {
    auto best_sol = dm.population.best_feasible();
    // also scour through the external solution queue to potentially
    // nab better solutiions early
    ...
    // TODO: try running CPUFJ in a thread as well?
    ...
    // branch_and_bound_settings.num_threads     = 4;
