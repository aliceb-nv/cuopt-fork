    ...
    if (run_early_cpufj) {
      ...
      auto incumbent_callback =
        [presolver_ptr, mip_callbacks, no_bound](
          ...
          callback_info.origin = (uint32_t)internals::mip_solution_origin_t::CPU_FEASIBILITY_JUMP;
                                   ...
                                   mip_solver_settings_t<i_t, f_t> const& settings_const)
{
  try {
    ...
    f_t no_bound = problem.presolve_data.objective_scaling_factor >= 0 ? (f_t)-1e20 : (f_t)1e20;
    if (run_early_fj) {
      auto publish_early_fj_solution = [&early_best_objective,
                                        &early_best_user_obj,
      ...
      auto early_cpufj_callback = [publish_early_fj_solution](f_t solver_obj,
                                                              f_t user_obj,
                                                              const std::vector<f_t>& assignment) {
        publish_early_fj_solution(
          solver_obj, user_obj, assignment, internals::mip_solution_origin_t::CPU_FEASIBILITY_JUMP);
      };
      auto early_gpufj_callback = [publish_early_fj_solution](f_t solver_obj,
                                                              f_t user_obj,
                                                              const std::vector<f_t>& assignment) {
        publish_early_fj_solution(
          solver_obj, user_obj, assignment, internals::mip_solution_origin_t::FEASIBILITY_JUMP);
      };

      // Start early CPUFJ on original problem (will restart on presolved problem after Papilo)
      early_cpufj = std::make_unique<detail::early_cpufj_t<i_t, f_t>>(
        op_problem, settings.get_tolerances(), early_cpufj_callback);
      early_cpufj->start();
      CUOPT_LOG_INFO("Started early CPUFJ on original problem");

      // Start early GPU FJ (uses GPU while CPU is busy with Papilo)
      early_gpufj = std::make_unique<detail::early_gpufj_t<i_t, f_t>>(
        op_problem, settings, early_gpufj_callback);
      early_gpufj->start();
